...
def cross_val(X, y, model=LinearRegression(), name='Linear',
    ...
    num_of_col = len(X.columns)
    model_coefs = np.array([0]*num_of_col)

    # set dummy index to the end if there is none.
    if not dummy_idx:
        dummy_idx = num_of_col
    ...
    return get_predicts(X.copy(), model=model, poly=poly, scale=scale,
                        dummy_idx=dummy_idx)
...
def split_poly_scale_join(matrices, dummy_idx, poly, scale):
    ...
    for matrix in matrices:
        ...
        matrix_num, matrix_dummies = np.split(matrix, [dummy_idx], axis=1)

        # add polynomial features to numerical attributes
        if poly:
            matrix_num = (PolynomialFeatures(poly).
                            fit_transform(matrix_num))
        # scale numerical attributes
        if scale:
            matrix_num = (StandardScaler().
                            fit_transform(matrix_num))
        # Join numerical and dummy attributes together again
        adjusted_matrices.append(
            np.concatenate((matrix_num, matrix_dummies), axis=1))
